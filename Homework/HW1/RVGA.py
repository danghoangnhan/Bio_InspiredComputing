...
def crossover(fun, df, popSize):
    ...
    for i in range(halfPopSize):
        offsprings.loc[2 * i] = (alpha[2 * i] * df.loc[2 * i % halfPopSize] + (1 - alpha[2 * i]) * df.loc[
            (2 * i + 1) % halfPopSize]).tolist()
        offsprings.loc[2 * i + 1] = ((1 - alpha[2 * i + 1]) * df.loc[2 * i % halfPopSize] + alpha[2 * i + 1] * df.loc[
            (2 * i + 1) % halfPopSize]).tolist()
...
def mutation(fun, df, popSize):
    ...
    for i in range(popSize):
        if np.random.rand() < 0.05:
            ...
            df.loc[i, variables[mutIndex]] = df.loc[i, variables[mutIndex]] + \
                                             np.random.normal(0, df.loc[:, variables[mutIndex]].var(), popSize)[i]
