...
class Run(object):
    ...
    def __init__(self, generations, pop_size):
        ...
        # Circle creation
        for city in global_truck_list:
            self.canvas_current.create_oval(city.graph_x * 8 - 5, city.graph_y * 4 - 5, city.graph_x * 8 + 5,
                                            city.graph_y * 4 + 5, fill='yellow')
            self.canvas_best.create_oval(city.graph_x * 8 - 5, city.graph_y * 4 - 5, city.graph_x * 8 + 5,
                                         city.graph_y * 4 + 5, fill='yellow')
